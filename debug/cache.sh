...
function set_arg_val(){
  local key="${CONFIG_ARG_PREFIX}_${1}"
  local val="${2}"
  dic+=([$key]=$value)
}
function get_val_by_key(){
    local temp=
    local key=
    local val=
...
local FILE_PATH=$(path_resolve $PROJECT_PATH $FILE_PATH)
local CUSTOM_FILE=
  if [ -n "${2}" ]
  then
